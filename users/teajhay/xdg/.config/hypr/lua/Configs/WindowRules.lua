...
hl.window_rule({
  name = "email-to-special",
  match = { initial_class = "org.mozilla.Thunderbird" },
  workspace = "special:mailbox silent",
...
-- layed out as the { "rule name", {match field = "RegEx"}, {optional additonal rules such as centering, size and etc.
-- Enter here as docs say and function will parse it, otherwise discard it if empty}}, All rules here will float
...
local float_rules = {
  ...
  {
    "vscodium-secondary-window",
    { title = "negative:(.*codium.*|.*VSCodium.*)", class = "(codium|codium-url-handler|VSCodium)" },
  },
  {
    "heroic-secondary-window",
    { title = "negative:(Heroic Games Launcher)", class = "^(com.heroicgameslauncher.hgl)$" },
  },
  ...
  { "pipewire-control", { class = "^(pavucontrol|org.pulseaudio.pavucontrol|com.saivert.pwvucontrol)$" } },
  {
    "network-editor",
    { tag = "network" },
    { move = { "monitor_w * 0.99 - window_w", "60" } },
  },
