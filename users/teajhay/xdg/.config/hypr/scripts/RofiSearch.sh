...
Search_Engine=$(sed -n 's/.*Search_Engine = "\(.*\)".*/\1/p' $HOME/.config/hypr/UserConfigs/01-UserDefaults.lua)
...
config_content=$(grep -E '^\s*local\s+[A-Za-z_][A-Za-z0-9_]*\s*=\s*".*"\s*$' "$config_file" \
    | sed -E 's/^\s*local\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*/export \1=/')
