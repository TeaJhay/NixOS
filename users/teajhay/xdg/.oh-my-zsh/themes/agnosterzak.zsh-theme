...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment magenta white "%{$fg_bold[#bf262e]%(!.%{%F{#bf262e}%}.)%}$USER@%m%{$fg_no_bold[#bf262e]%}"
  else
    prompt_segment yellow '#bf262e' "%{$fg_bold[#bf262e]%(!.%{%F{#bf262e}%}.)%}@$USER%{$fg_no_bold[#bf262e]%}"
  fi
}
...
prompt_time() {
  prompt_segment 12 white "%{$fg_bold[white]%}%D{%a %e %b - %H:%M}%{$fg_no_bold[white]%}"
}
